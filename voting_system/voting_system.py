...
# 获取指定行的数据
  ...
  data = row_data.iloc[0] #读取KNN预测结果
  element_count1 = w1*data.count(element1) #Distinction的次数
  element_count2 = w1*data.count(element2) #Pass的次数
  element_count3 = w1*data.count(element3) #Withdrawn的次数
  element_count4 = w1*data.count(element4) #Fail的次数
...
#增加权重
  ...
  numbers = [element_count1, element_count2, element_count3, element_count4]  # 假设这是你的数字列表
  #labels = ['A', 'B+', 'B', 'B-', 'C', 'D', 'F']  # 假设这是与数字对应的标签列表
  labels = ["Distinction","Pass","Withdrawn","Fail"]  # 假设这是与数字对应的标签列表
...
data = Weight_output
filename = 'Weight_output2.csv'  # 输出的CSV文件名
...
input_file = 'Weight_output2.csv'  # 输入的CSV文件名
output_file = 'Weight_output2.csv'  # 输出的CSV文件名
...
df = pd.read_csv('True_level2.csv')
column_data = df['Level']

df = pd.read_csv('Weight_output2.csv')
